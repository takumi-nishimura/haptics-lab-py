...
fps = 240
opti_client = OptiClient(server_address="127.0.0.1", client_address="127.0.0.1")
    ...
    try:
        ...
        if len(opti_client.mocap_data.keys()) > 1:
            if init_pose:
                ...
                print(
                    f"Relative Pos: {relative_pos[rigid_body_id]}, Relative Rot: {opti_client.to_euler_from_quat_list(list(relative_rot[rigid_body_id]))}"
                )
            else:
                init_pose = dict()
                init_pose[rigid_body_id] = opti_client.mocap_data[rigid_body_id]
